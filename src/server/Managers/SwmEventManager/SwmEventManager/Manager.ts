...
import { EntityInputData } from "../SwmEventInputData";
import { Entities } from "../SwmEvents";
import type {
...
export class EntityManager {
	...
	async getAll(rawInputs: EntityGetAllRawInputs): Promise<Entities> {
		...
		const inputData = new EntityInputData(this.entityType, updatedInputs);
		inputData.validateGetAllInputs();

		const rawEntities = await this.db.getAll(inputData);
	...
	async getByIds(rawInputs: EntityGetByIdsRawInputs): Promise<Entities> {
		...
		const inputData = new EntityInputData(this.entityType, updatedInputs);
		inputData.validateGetByIdsInputs();

		const rawEntities = await this.db.getByIds(inputData);
		const entities = await this.getEntities(rawEntities, inputData);
		return entities;
	}
	...
	async getById(rawInputs: EntityGetByIdRawInputs): Promise<Entity> {
		...
		const inputData = new EntityInputData(this.entityType, updatedInputs);
		inputData.validateGetByIdInputs();

		const rawEntity = await this.db.getById(inputData);
	...
	async getByName(rawInputs: EntityGetByNameRawInputs): Promise<Entity> {
		...
		const rawEntity = await this.db.getByName(inputData);
		const entity = new Entity(rawEntity);
		entity.loadRelatedData(inputData, this.imageManager, this.metadataManager);
	...
	async getEntities(rawEntities: RawEntity[], inputData: EntityInputData) {
